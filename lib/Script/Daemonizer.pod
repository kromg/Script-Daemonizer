...
Pre-release. Code lacks comments and a bit of testing. Doc may not be complete. 
...
=item 7.* it ties STDOUT and STDERR to Syslog using  L<< Tie::Syslog >> (if
...
(guess what? See L<< /ADVANCED USAGE >> for details) but this will happen B<<
before >> anything else (think of this as step 0). If you need to drop
...
handle SIGHUP might be restarting, for example (see L<<
perlipc|http://perldoc.perl.org/perlipc.html#Handling-the-SIGHUP-Signal-in-Daemons >>
for details). 
...
You must set up restart() as the handler of SIGHUP yourself. See L<< /restart()
>> for details.
...
Default is to tie STDOUT and STDERR with to facility LOG_LOCAL0, priorities
...
=item * an C<fuser> (fuser(1)) on the pidfile will reveal the daemon's pid. If 
...
syslog (L<< Sys::Syslog >>), facility, priority).
...
=item * Use Tie::Log4Perl as an alternative to Tie::Syslog.
