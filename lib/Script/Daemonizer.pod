...
daemonize()-ing. The only way to get rid of them is to close() them all just 
...
after the second fork(). This allows a race condition between the first and the
second flock(), but it should not be a problem, since the lock could be as well
tried just after the second fork(). The reason I left the first attempt in place
